...
public class GlobalProjectorManager : MonoBehaviour {
	...
	public bool useNonAllocCulling = false;
	...
	bool _anyTriggersAlive = false;
	bool _shouldCheckTriggers = false;
	
	Plane[] _mainCameraPlanes;
	...
	void ForceShadowDummyRebuild(List<ShadowProjector> projectors) {
		for (int n = 0; n < _ShadowProjectors.Count; n++) {
			var shadowProjector = _ShadowProjectors[n];
	...
	void CalculateShadowBounds(Camera targetCamera, List<ShadowProjector> projectors) {
		...
		for (int n = 0; n < projectors.Count; n++) {
			...
			case ProjectionCulling.ProjectorBounds: 
			{
				...
				if (!GeometryUtility.TestPlanesAABB(_mainCameraPlanes, shadowProjector.GetBounds())) {
					shadowProjector.Discard(true);
					continue;
				} else {
					shadowProjector.Discard(false);
			...
			case ProjectionCulling.ProjectionVolumeBounds: 
			{
				...
				if (!IsProjectionVolumeVisible(_mainCameraPlanes, shadowProjector)) {
					shadowProjector.Discard(true);
					continue;
				} else {
					shadowProjector.Discard(false);
	...
	void CheckMainCameraPlains() {
		if (!_cameraPlainsCalculated) {
			if (useNonAllocCulling) {
				FSPUtils.CalculateFrustumPlanes(_mainCameraPlanes, Camera.main);
			}
			else {
				_mainCameraPlanes = GeometryUtility.CalculateFrustumPlanes(Camera.main);
			}
	...
	void OnLightPreCull() {		
		for (int n = 0; n < _LightProjectors.Count; n++) {
			var lightProjector = _LightProjectors[n];
